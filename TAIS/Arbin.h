...
public:
	...
	*/
	...
	/** En todos estos métodos devolvemos un puntero a una lista. Funciona porque ret es una lista en el Heap.
	 *  OJO, esto significa que debemos borrar la lista que devolvamos.
	 */
	...
	bool operator<(const Arbin<T>& rhs) const {
		return comparaAux1(ra, rhs.ra);
	}
	bool operator>(const Arbin<T>& rhs) const {
		return comparaAux2(ra, rhs.ra);
	}
	static bool comparaAux1(Nodo* r1, Nodo* r2) {
		if ((r1 == nullptr))
			// En el if anterior nos aseguramos de que r1 != r2. Si uno es NULL, el
			// otro entonces no lo será, luego son distintos.
			return true;
		else if (r1->elem < r2->elem)
			return true;
		return false;
	}
	static bool comparaAux2(Nodo* r1, Nodo* r2) {
		if ((r1 == nullptr))
			// En el if anterior nos aseguramos de que r1 != r2. Si uno es NULL, el
			// otro entonces no lo será, luego son distintos.
			return true;
